...
FetchHook useFetchCatagories() {
  final catagoriesItem = useState<List<Catagorymodel>?>([]);
  ...
  Future<void> fetchData() async {
    ...
    try {
      final url = '$baseurl/category/getrandomcatagory';
      print('API URL: $url');
      final response = await http.get(Uri.parse(url));
      print(response.statusCode);

      if (response.statusCode == 200) {
        catagoriesItem.value = catagorymodelFromJson(response.body);
      } else {
