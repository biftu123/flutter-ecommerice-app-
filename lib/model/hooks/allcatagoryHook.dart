...
FetchHook allFetchCatagories() {
  final catagoriesItem = useState<List<Allcatagory>?>([]);
  ...
  Future<void> fetchData() async {
    ...
    try {
      final url = '$baseurl/category/getcatagory';
      print('API URL: $url');
      final response = await http.get(Uri.parse(url));
      print(response.statusCode);

      if (response.statusCode == 200) {
        catagoriesItem.value = allcatagoryFromJson(response.body);
      } else {
